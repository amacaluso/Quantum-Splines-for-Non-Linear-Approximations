...
def coeff_splines_estimation(x, y, label, saving=True):
    """
    Given a list of points (x,y), creates a series of 2x2 linear systems
    and estimates the B-Spline coefficients with both quantum and classical
    approaches. If saving=True, it saves results at the specified path

    :param x: (float) list of x coordinates
    :param y: (float) list of y coordinates
    :param label: (string) label of the output file
    :param saving: (bool) whether to save the output in csv format
    :return: (pandas.Dataframe) dataframe with the end point of each x-interval,
             the quantum and classical coefficients, and the fidelity of the HHL in each interval
    """
...
def estimate_function(data, function, label, c=0, step=0.05):
    """
    Takes a dataframe with intervals and coefficients and evaluates the value of the
    specified function.

    :param data: (pandas.Dataframe) the output of the function coeff_splines_estimation
    :param function: (object) the objective function
    :param label: (string) label of the output file
    :param c: (float) shifting parameter
    :param step: (float) interval width
    :return: (pandas.Dataframe) dataframe with the list of x and y coordinates,
             and the estimates of hybrid, full and classical spline
    """
